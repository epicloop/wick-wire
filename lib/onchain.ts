...
export function buildEvents(input: {
  ...
  for (const w of pairedWindow ?? []) {
    ...
    ev.push({
      ...
      kind: "paired_token",
      time: w.peakTime,
      title: `$${w.pool.otherSymbol} (${short(w.pool.otherAddress)}), paired against ${stock.token}, ${mv == null ? "moved" : `${mv >= 0 ? "+" : "−"}${Math.abs(mv).toFixed(0)}%`} over the window; ${usd(w.volUsd)} traded in its pool (${Math.round((w.volUsd / (windowVol || 1)) * 100)}% of the main pool's volume)`,
  ...
  if (!pairedWindow) for (const p of ps.filter((p) => p.paired && p.volume24hUsd >= 10_000 && p.volume24hUsd / totalVol >= 0.02).sort((a, b) => b.volume24hUsd - a.volume24hUsd).slice(0, 3)) {
    ...
    ev.push({
      ...
      kind: "paired_token",
      time: toSec,
      title: `$${p.otherSymbol} (${short(p.otherAddress)}), paired against ${stock.token}, ${mv == null ? "moved (24h change unavailable)" : `${mv >= 0 ? "+" : "−"}${Math.abs(mv).toFixed(0)}% in 24h`}; its pool is ${share.toFixed(0)}% of ${stock.token} DEX volume`,
